...
class InternVLImageEmbeddingInputs(TensorSchema):
    ...
    data: Annotated[
        Union[torch.Tensor, list[torch.Tensor]], TensorShape("n", "f", "h")
    ]
...
class BaseInternVLProcessor(ABC):
    ...
    def resolve_min_max_num(
        ...
        *,
        ...
        min_dynamic_patch = (
            ...
            if min_dynamic_patch is None
            else min_dynamic_patch
        )
        max_dynamic_patch = (
            self.max_dynamic_patch
            if max_dynamic_patch is None
            else max_dynamic_patch
        )
        dynamic_image_size = (
        ...
        use_thumbnail = (
            self.use_thumbnail if use_thumbnail is None else use_thumbnail
        )
    ...
    def _make_batch_input(
        self, input_item: Optional[Union[Any, list[Any]]] = None
    ):
...
class InternVLMultiModalProcessor(
    ...
    def _get_mm_fields_config(
        ...
        if self.info.supports_video:
            video_num_patches = hf_inputs.get(
                "video_num_patches", torch.empty(0)
            )
...
class InternVLChatModel(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.skip_load_llm = (
            ...
            and vllm_config.kv_transfer_config.is_encoder_only
        )
    ...
    def _parse_and_validate_video_input(
        ...
        if pixel_values_flat_video is not None:
            ...
            pixel_values_flat_video = flatten_bn(
                pixel_values_flat_video, concat=True
            )
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> dict:
        ...
        for input_key in kwargs:
            ...
            if (
                input_key in ("pixel_values_flat_video",)
