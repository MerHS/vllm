...
class Phi4MMProcessingInfo(BaseProcessingInfo):
    ...
    def _compute_num_image_tokens(
        ...
        assert (
            vit_image_size // vit_patch_size % token_compression_factor == 0
        ), (
    ...
    def get_dummy_mm_data(
        ...
        mm_data = {
            ...
            "audio": self._get_dummy_audios(
                length=_AUDIO_MAX_SOUNDFILE_SIZE, num_audios=num_audios
            ),
        }
...
class Phi4MMForCausalLM(nn.Module, SupportsLoRA, SupportsMultiModal):
        ...
        # condition is only for v0 compatibility.
            ...
            if image_input is None and audio_input is None:
                ...
                inputs_embeds = self.get_input_embeddings_v0(
                    input_ids, image_input=image_input, audio_input=audio_input
                )
                input_ids = None
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> None:
        skip_prefixes = ["model."] if self.skip_load_llm else None

        loader = AutoWeightsLoader(
            self, skip_substrs=["lora"], skip_prefixes=skip_prefixes
        )
