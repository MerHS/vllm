...
class SampleRequest:
    ...
    multi_modal_data: Optional[Union[MultiModalDataDict, dict, list[dict]]] = (
        None
    )
...
class BenchmarkDataset(ABC):
    ...
    def maybe_oversample_requests(
        ...
        if no_oversample:
            logger.info(
                "Skipping oversampling. Total samples: %d.", len(requests)
            )
            return
        ...
        if len(requests) < num_requests:
            ...
            logger.info(
                "Oversampled requests to reach %d total samples.", num_requests
            )
...
class RandomDataset(BenchmarkDataset):
    ...
    def get_sampling_params(
        ...
        input_lens = self._rng.integers(
            input_low, input_high + 1, size=num_requests
        )
...
class RandomMultiModalDataset(RandomDataset):
    ...
    def generate_mm_item(
        ...
        if self.map_config_to_modality(mm_item_config) == "image":
            return process_image(
                self.generate_synthetic_image(
                    mm_item_config[1], mm_item_config[0]
                )
            )
            ...
            raise ValueError(
                f"Invalid multimodal item configuration: {mm_item_config}"
            )
    ...
    def get_mm_item_sampling_params(
        ...
        allowed_modalities = {
            self.map_config_to_modality(cfg) for cfg in bucket_config
        }
        limit_mm_per_prompt = {
            k: v
        ...
        if not limit_mm_per_prompt:
            raise ValueError(
                "No valid limits for modalities present in bucket_config."
            )
    ...
    def get_mm_item_iterator(
        ...
        modality_counter = {
            self.map_config_to_modality(k): 0 for k in bucket_config
        }
        ...
        while sum(modality_counter.values()) < request_num_mm_items:
            ...
            if modality_counter[modality] < limit_mm_per_prompt[modality]:
                ...
                if all(v == 0 for v in bucket_config_copy.values()):
                    logger.warning(
                        "Exhausted all multimodal items of modality %s",
                        modality,
                    )
                    break
...
def get_samples(args, tokenizer) -> list[SampleRequest]:
    ...
    if args.dataset_name == "custom":
        ...
        dataset_mapping = {
            ...
            "random-mm-file": lambda: RandomMultiModalDataset(
                random_seed=args.seed, dataset_path=args.dataset_path
            ).sample_file(
                tokenizer=tokenizer,
                num_requests=args.num_prompts,
...
class VisionArenaDataset(HuggingFaceDataset):
    ...
    DEFAULT_OUTPUT_LEN = 128
    SUPPORTED_DATASET_PATHS = {
        "lmarena-ai/VisionArena-Chat": lambda x: x["conversation"][0][0][
            "content"
        ],
        "lmarena-ai/vision-arena-bench-v0.1": lambda x: x["turns"][0][0][
            "content"
        ],
    }
    IS_MULTIMODAL = True
        ...
        **kwargs,
        ...
        def _generate_exact_length_tokens(target_length: int) -> list[int]:
            ...
            tokens = np.random.randint(
                0, vocab_size, size=target_length
            ).tolist()
