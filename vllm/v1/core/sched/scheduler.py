...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        if vllm_config.kv_transfer_config is not None:
            ...
            self.is_encode_sender = (
                vllm_config.kv_transfer_config.is_encode_sender
            )
        else:
            self.is_encoder_only = False
            self.is_encode_sender = False

        encoder_cache_manager_class = (
            EncoderBlockCacheManager
        ...
        logger.info(
            "Initialized encoder cache manager with size %s", encoder_cache_size
        )
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                if request.status == RequestStatus.WAITING_FOR_REMOTE_MMS:
                    is_ready = self.encoder_cache_manager.recv_finished(request)
                    if is_ready:
                        request.status = RequestStatus.WAITING
                    else:
                        logger.debug(
                ...
                required_encoder_inputs, receiving_mm_hashes = (
                    self.encoder_cache_manager.get_required_encoder_inputs(
                        request
                    )
                )
                ...
                if self.connector is not None:
                    ...
                    remote_encoder_inputs = (
                        self.connector.get_available_remote_encoder_inputs(
                            request,
                            required_encoder_inputs,
                        )
                    )
                    load_mm_async = bool(
                        remote_encoder_inputs or len(receiving_mm_hashes) > 0
                    )
        ...
        structured_output_request_ids, grammar_bitmask = (
            self.get_grammar_bitmask(self.running, scheduled_spec_decode_tokens)
        )
        ...
        for req_id, encoder_input_ids in scheduled_encoder_inputs.items():
            for input_id in encoder_input_ids:
                ...
                scheduled_encoder_segments[mm_hash] = (
                    self.encoder_cache_manager.get_segments(mm_hash)
                )
        ...
        for mm_hash in request.mm_hashes:
            mm_segments.append(
                (
                    ...
                    [
                        (s.address, s.size)
                        for s in self.encoder_cache_manager.get_segments(
                            mm_hash
                        )
                    ],
                )
        ...
        * if finished_sending: free the blocks
        ...
        for req_id in kv_connector_output.finished_recving or ():
            ...
            if (
                ...
                == RequestStatus.WAITING_FOR_REMOTE_MMS
            ):
